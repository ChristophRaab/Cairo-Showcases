...
np.random.seed(0)
torch.manual_seed(0)
from data_generation.helpers import features_dataset,init_weights,train_resnet,create_feature_extractor,train_resnet_metric,print_learning
...
def make_parser():
    ...
    parser.add_argument('--batch_size', type=int, default=86, help="Batch Size of Train loaders")
...
def train_representation(args):
    ...
    features_extractor = nn.Sequential(create_feature_extractor(),nn.Flatten(),nn.Linear(2048,args.bottleneck_dim)).to(args.cuda)
    ...
    if args.mode != "unsupervised":
        for i in range(args.num_epochs):
            ...
            print_learning(i,avg_acc,train_dataset_size,avg_loss,train_loader_size,best_acc)
        ...
        for i in range(args.num_epochs):
            with torch.set_grad_enabled(False):
                ...
                ky = cluster.AgglomerativeClustering(n_clusters=num_classes).fit_predict(features)
            ...
            with torch.set_grad_enabled(True):
                avg_loss = avg_acc  = 0.0
                for (xs,ys) in train_loader:
                    optimizer,features_extractor,loss_func,avg_loss,avg_acc= train_resnet(xs,ys,features_extractor,classifier,optimizer,avg_loss,avg_acc,args.cuda,weight)
                print_learning(i,avg_acc,train_dataset_size,avg_loss,train_loader_size,best_acc)
                best_model = copy.deepcopy([features_extractor]) if avg_acc > best_acc else best_model
